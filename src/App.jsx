...
const LayoutWrapper = ({ children, isHomePage = false }) => (
  <div className={`min-h-screen flex flex-col font-['Courier_New',_monospace] text-right items-end pr-8`}>
...
const Navigation = () => {
  const { navigateWithTransition } = React.useContext(PageTransitionContext);
  return (
    <div className="fixed top-0 left-0 w-full bg-transparent flex flex-col items-end py-4 z-40 font-['Courier_New',_monospace] text-gray-600 pr-8">
      ...
      <nav className="flex justify-end w-full max-w-md text-lg font-light mt-2">
        <TransitionLink to="/furniture" className="hover:underline text-right">
          Furniture
        </TransitionLink>
      </nav>
    </div>
...
const Footer = () => (
  <footer className="w-full py-12 px-8 text-right mt-16 font-['Courier_New',_monospace] text-gray-600">
  ...
  return (
    <LayoutWrapper isHomePage={true}>
      <div className="relative w-full h-screen">
        <img src={`${import.meta.env.BASE_URL}images/frontpage_images/all-1.JPEG`} alt="Industrial Furniture" className="w-full h-full object-cover" style={{ filter: "opacity(70%)" }} />
        <div className="absolute top-0 right-0 w-full pt-8 pr-8 text-right">
...
const FurniturePage = () => (
  <LayoutWrapper>
    <Navigation />
    <div className="flex justify-end pt-28 pr-8">
      <div className="flex flex-col items-end gap-12 max-w-xl pb-16">
        {furnitureList.map((item) => (
          <TransitionLink to={`/furniture/${item.id}`} key={item.id}>
            <div className="relative w-80 group">
              ...
              <img src={item.images[0]} alt={item.name} className="w-full h-auto object-cover" style={{ filter: "opacity(70%)" }} />
...
const FurnitureDetail = () => {
  ...
  if (!item) return <div className="p-6 text-right font-light text-gray-600">Furniture not found</div>;

  return (
    <LayoutWrapper>
      <Navigation />
      <div className="max-w-3xl ml-auto w-full pt-28 pr-8">
        <div className="flex flex-col items-end">
          {item.images.map((img, idx) => (
            <img
              ...
              alt={`${item.name} ${idx + 1}`}
              className="mb-8"
              style={{ width: '40%', filter: 'opacity(70%)' }}
            />
          ))}
